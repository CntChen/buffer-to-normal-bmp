...
function from32To24(bgraBuffer) {
    ...
    var pixelByteUnit = 4;
    var pixelWidth = Number.parseInt(bgraBuffer.length / pixelByteUnit);
...
function from16To32(bgraBuffer) {
    ...
    var as = [];
    for (var i = 0; i < pixelWidth; i++) {
        var firstBufer = bgraBuffer.slice(pixelByteUnit * i + 0, pixelByteUnit * i + 1);
        var secondBuffer = bgraBuffer.slice(pixelByteUnit * i + 1, pixelByteUnit * i + 2);
